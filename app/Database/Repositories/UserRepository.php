...
//            throw new Exception('Invalid Email');
     ...
     * @access public
     * @param string $email, string $forgotKey
     * @return int
     */
    function send_forgot_password_link($email)
    {
        $forgotKey=randomPassword();
        $this->set_szLoginEmail(trim(sanitize_all_html_input($email->email)));
        if ($this->error === true)
        {
            $res["site_response"]=array("response"=>"ERROR");
            $res["site_message"]=array("message"=>reset($this->arErrorMessages));
        ...
        $query="
			...
			FROM
				".__DBC_SCHEMATA_USERS__."
			WHERE
				szEmail = '".mysql_real_escape_string($this->szLoginEmail)."'
                        AND
                            isDeleted='0'
        ...
        if( ($result = $this->exeSQL($query)) )
        {
            if($this->iNumRows>0)
            {
                ...
                $query1="
					UPDATE
						".__DBC_SCHEMATA_USERS__."
					SET
						szForgotPasswordKey='".mysql_real_escape_string($forgotKey)."',
						keyDateTime = NOW()
					WHERE
						id = '".(int)$row['id']."'
				";
                ...
                {
                    $userId=$row['id'];
                    $userAry=$this->fetchUserDetailById($userId);
                    $kEmail=new cEmail();
                    $emailAry=$kEmail->getEmailTemplateByKey('__FORGOT_EMAIL__');
                    $email=trim($this->szLoginEmail);
                    $subject=$emailAry['subject'];
                    $url=__BASE_URL__.'/resetpassword/'.$forgotKey.'/';
                    $message=str_replace('szLink','<a href=\''.$url.'\'>Click here</a>',$emailAry['description']);
                    $message=str_replace('szTextLink', __MAIN_SITE_URL__.'/resetpassword/'.$forgotKey.'/' ,$message);
                    $message=str_replace('szName', $userAry['szFirstName'],$message);
                    sendEmail($email,$subject,$message,$userId);

                    $res["site_response"]=array("response"=>"SUCCESS");
                    $res["site_message"]=array("message"=>"Your password reset link has been sent.");
                    return $res;
                }
            }
            else
            {
                $res["site_response"]=array("response"=>"ERROR");
                $res["site_message"]=array("message"=>"Invalid email address");
                return $res;
            }
        }
        else
